...
import { SlidersHorizontal, ChevronDown, Loader2, X, Filter } from 'lucide-react';
...
export function ShopPage() {
  ...
  const getSubcategories = (parentSlug: string) => {
    if (!parentSlug || parentSlug === 'all') return [];
    ...
    // Otherwise filter categories by parent_id
    return categories.filter(cat => cat.parent_id === parentCat.id);
  };
  ...
  useEffect(() => {
    ...
    fetchProducts();
  }, [selectedCategory, selectedSubcategory, sortBy, currentPage, priceRange]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:hidden mb-4">
            <motion.button
              ...
              className="w-full flex items-center justify-center gap-2 bg-purple-600 text-white py-3 px-4 rounded-xl shadow-lg"
            >
              ...
              <>
                ...
                >
                  <div className="p-6">
                    ...
                    {/* Category Filter */}
                    <div className="mb-6">
                      <h4 className="mb-3 text-gray-700 font-medium">Category</h4>
                    ...
                    {subcategories.length > 0 && (
                      <div className="mb-6">
                        <h4 className="mb-3 text-gray-700 font-medium">Subcategory</h4>
                        <div className="space-y-2 pl-2 border-l-2 border-purple-200">
                          <label className="flex items-center space-x-2 cursor-pointer group">
                            <input
                              ...
                              className="w-4 h-4 text-purple-600"
                            />
                            <span className="group-hover:text-purple-600 transition-colors text-sm">
                              All in {categories.find(c => c.slug === selectedCategory)?.name}
                            </span>
                          </label>
                          {subcategories.map((subcat) => (
                            <label key={subcat.id} className="flex items-center space-x-2 cursor-pointer group">
                              <input
                                type="radio"
                                name="mobile-subcategory"
                                checked={selectedSubcategory === subcat.slug}
                                onChange={() => setSelectedSubcategory(subcat.slug)}
                                className="w-4 h-4 text-purple-600"
                              />
                              <span className="group-hover:text-purple-600 transition-colors text-sm">
                    ...
                    {/* Price Range */}
                    <div className="mb-6">
                      <h4 className="mb-3 text-gray-700 font-medium">Price Range</h4>
          ...
          >
            <div className="bg-white p-6 rounded-xl shadow-sm sticky top-24">
              ...
              {/* Subcategory Filter - Only shown when parent category has subcategories */}
              {subcategories.length > 0 && (
                <div className="mb-6">
                  <h4 className="mb-3 text-gray-700 font-medium">Subcategory</h4>
                  <div className="space-y-2 pl-2 border-l-2 border-purple-200">
